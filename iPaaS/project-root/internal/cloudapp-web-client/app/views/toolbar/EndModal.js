...
class EndModal extends React.Component {
  ...
  onEnd = () => {
    ...
    const sessionId = Number(search.get('session_id'))
    WorkTaskService.closeWorkTask(userId, workTaskId, sessionId).then(() => {
      this.onClose()
      setTimeout(() => {
        window.close()
      }, 200)
  ...
  render() {
    return (
      <div className="modal is-active" id="close-worktask-confirm-modal">
        <div className="modal-background" />
        <div className="modal-content">
          <div className="modal-card">
            <header className="modal-card-head">
              <p className="modal-card-title">退出</p>
              <span
                onClick={this.onClose}
                id="close-worktask-modal-close"
                aria-label="close">
                <svg
                  t="1564628203999"
                  className="icon"
            ...
            <section className="modal-card-body">确定退出该应用？</section>
