...
func StartSession(c *gin.Context, opts ...Option) {
	conf := new(config)
	...
	err := bindStartSessionRequest(req, c)
	if err = response.BadRequestIfError(c, err, util.InvalidArgumentErrResp); err != nil {
		logger.Warnf("bind start session request failed, %v", err)
		return
	}

	err, errResp := validator.ValidateStartSessionRequest(req)
	if err = response.BadRequestIfError(c, err, errResp); err != nil {
		logger.Warnf("validate admin start session request failed, %v", err)
		return
	}
	...
	err = with.DefaultTransaction(c, func(ctx context.Context) error {
		...
		if sessionWithInstance.Session.Status != schema.SessionPowerOff {
			allowed = false
			return nil
		}

		e = dao.UpdateSessionStatus(ctx, sessionId, []schema.SessionStatus{schema.SessionPowerOff}, schema.SessionPoweringOn)
		if e != nil {
			return fmt.Errorf("update session [%s] to status [POWERING UP] failed, %w", sessionId, e)
		}

		return state.Cloud.StartInstance(sessionWithInstance.Session.Zone, sessionWithInstance.Instance.InstanceId)
	})
	...
	if !allowed {
		err = fmt.Errorf("session [%s] in state [%s] now allowed to start", sessionWithInstance.Session.Id, sessionWithInstance.Session.Status)
		_ = response.ForbiddenIfError(c, err, response.WrapErrorResp(ensureForbiddenStartSessionErrorCode(isAdminApi(c.FullPath())), err.Error()))
		logger.Warn(err)
		return
	}
...
func bindStartSessionRequest(req *session.PowerOnRequest, c *gin.Context) error {
...
func ensureForbiddenStartSessionErrorCode(isAdmin bool) string {
	if isAdmin {
		return common.ForbiddenSessionAdminStart
	}

	return common.ForbiddenSessionUserStart
}
