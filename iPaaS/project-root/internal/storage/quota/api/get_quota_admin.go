...
func (q *Quota) GetStorageQuotaAdmin(ctx *gin.Context) {
	userID, _, err := q.GetUserIDAndAKAndHandleError(ctx)
	if err != nil {
		return
	}
	logger := logging.GetLogger(ctx).With("func", "GetStorageQuotaAdmin", "RequestId", ctx.GetHeader(common.RequestIDKey), "UserId", userID)

	userIDStr := ctx.Param(UserIDKey)
	if userIDStr == "" {
		msg := fmt.Sprintf("Error parsing userID: %v,got: %v", err, userIDStr)
		logger.Infof(msg)
