...
import (
	...
	"github.com/yuansuan/ticp/common/project-root-api/storage/operationLog/api"
...
func (o *OperationLog) ListOperationLogApi(ctx *gin.Context) {
	...
	request := &api.ListOperationLogRequest{}
	...
	if request.FileTypes != "" {
		if !CheckFileType(request.FileTypes) {
			msg := fmt.Sprintf("invalid file type, got %s", request.FileTypes)
			logger.Info(msg)
	...
	if request.OperationTypes != "" {
		if !CheckOperationType(request.OperationTypes) {
			msg := fmt.Sprintf("invalid operation type, got %s", request.OperationTypes)
			logger.Info(msg)
	...
	if request.BeginTime != 0 {
		if !CheckTime(request.BeginTime) {
			msg := fmt.Sprintf("invalid begin time, should be unix timestamp, got: %v", request.BeginTime)
			logger.Info(msg)
	...
	if err != nil {
		msg := fmt.Sprintf("parse userID failed, userID: %v, err: %v", userID, err)
		logger.Infof(msg)
	...
	if !flag {
		msg := fmt.Sprintf("this user has no operation log, userID: %s", fmt.Sprint(id))
		logger.Infof(msg)
	...
	common.SuccessResp(ctx, &api.ListOperationLogResponseData{
