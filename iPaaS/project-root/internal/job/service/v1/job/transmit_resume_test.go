...
type TransmitResumeSuite struct {
	jobServiceSuite
...
func TestTransmitResume(t *testing.T) {
	suite.Run(t, new(TransmitResumeSuite))
}
...
func (tc *JobTransmitResumeTestCase) Run(s *TransmitResumeSuite) {
	s.Run(tc.name, func() {
		...
		// do
		err := s.jobSrv.TransmitResume(s.ctx, &jobtransmitresume.Request{
			JobID: tc.id,
...
func (s *TransmitResumeSuite) TestTransmitResume() {
	...
	tastCases := []JobTransmitResumeTestCase{
		{
			name: "normal",
			id:   "4ER",
			mockExpectFunc: func() {
		...
		},
		{
			...
			expectedError: common.ErrJobStateNotAllowTransmitResume,
			setReq: func() {
				s.allow = func(userID, jobUserID string) bool {
					return true
				}
