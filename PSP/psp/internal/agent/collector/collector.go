...
func NewDesc(fqName, help string, variableLabels []string) *prometheus.Desc {
	...
	name := prometheus.BuildFQName(metrics.Namespace, metrics.AgentSystem, fqName)
...
func executeCollector(collectorName string, collector Collector, ch chan<- prometheus.Metric, logger log.Logger) {
	...
	if err != nil {
		...
		level.Info(logger).Log("msg", "collector succeeded", "name", collectorName, "duration_seconds", duration.Seconds())
		success = 1
	}
