...
func ExportExcel(dataList []*ExportExcelSheetData) (*excelize.File, error) {
	f := excelize.NewFile()
	...
	for _, data := range dataList {
		...
		startColName := getColumnName(0, maxColumnRowNameLen)
		endColName := getColumnName(len(data.Headers)-1, maxColumnRowNameLen)
		startCol := getColumnRowName(startColName, 1)
		endCol := getColumnRowName(endColName, 1)
		err = f.SetColWidth(data.SheetName, string(startColName), string(endColName), 20)
		if err != nil {
			return nil, err
		}
		err = f.SetCellStyle(data.SheetName, startCol, endCol, headerStyle)
		if err != nil {
			return nil, err
		}
...
type ExportExcelSheetData struct {
	SheetName string
	Headers   []string
	Rows      [][]interface{}
}
