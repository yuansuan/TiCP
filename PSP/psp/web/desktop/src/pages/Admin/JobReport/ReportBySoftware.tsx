...
import { overviewDataApp as overviewData, 
  overviewPagingDataApp as overviewPagingData,
  detailDataApp as detailData, 
  detailPagingDataApp as detailPagingData,
  DATE_FORMAT,
} from './data'
...
// import { appWorker as worker, _eventName } from './MainThread'
import qs from 'qs'
...
export default observer(function ReportBySoftware(props) {
  ...
  const [exporting, setExporting] = useState<string | boolean>(false)
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (ref.current) ref.current.style.paddingRight = '1px'
    }, 3000)  
  ...
  async function fetchAppList(value) {
    if (value === 'all') {
      const res = await Http.get('/job/appNames')
      setAppList(res?.data || [])
    } else {
      const res = await Http.get('/job/appNames', {
        params: {
  ...
  useEffect(() => {
    eventEmitter.on('app_export_execl_success', ({message}) => {
      setExporting(false)
    })

    eventEmitter.on('app_export_execl_running', ({message}) => {
      setExporting('正在导出，请耐心等待')
    })

    eventEmitter.on('app_export_execl_error', ({message}) => {
      setExporting(false)
    })

    return () => {
      eventEmitter.off('app_export_execl_success')
      eventEmitter.off('app_export_execl_running')
      eventEmitter.off('app_export_execl_error')
    }
  }, [])
