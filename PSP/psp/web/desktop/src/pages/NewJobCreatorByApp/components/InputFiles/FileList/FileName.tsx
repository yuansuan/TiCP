...
interface IProps {
  ...
  upload: any
  isExpand: boolean
  myFileLoading: boolean
  copyOrUploadFilesFromFileManager: any
}
...
export const FileName = observer((props: IProps) => {
  ...
  const dirActions = [
    ...
    <Tooltip title='我的文件' key='myFile'>
      <span
        style={{
          pointerEvents: myFileLoading ? 'none' : 'auto',
          cursor: myFileLoading ? 'not-allowed' : 'pointer'
        }}>
        <HoverableIcon
          type='folder_mine'
          onClick={() => {
            ...
            copyOrUploadFilesFromFileManager(record.id, record.path)
          }}
        />
      </span>
    </Tooltip>,
