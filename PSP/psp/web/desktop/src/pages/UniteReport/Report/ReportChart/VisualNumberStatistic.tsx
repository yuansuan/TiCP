...
const title = '可视化会话数量统计'

@observer
export default class VisualNumberStatistic extends React.Component<IProps> {
          ...
          {this.loading ? (
            ...
            <>
              <LineChart
                type={this.type}
                title={`${title}`}
                data={this.chartData['number_status'] || []}
                unit={'个'}
                timeFormat={'MM-DD'}
              ...
              {/* <LineChart
                type={this.type}
                title={'作业等待人次情况（按天）'}
                data={this.chartData['job_wait_num_statistic'] || []}
                timeFormat={'MM-DD'}
                summaryPosition={['50%', '-8%']}
                summaryWidth={this.props.width + 'px'}
                unit={''}
