...
export const FileList = observer(function FileList({
  ...
  const [fetch, loading] = store.useRefresh(isSyncToLocal,userName)
  const [artificialClickLoading, setArtificialClickLoading] = useState(false)
  ...
  const { types, fileCount, dirCount } = state

  const deletable = isWidgetVisible('delete')
  const editable = isWidgetVisible('edit')
  const handleButtonClick = () => {
    setArtificialClickLoading(true)
    setTimeout(() => {
      setArtificialClickLoading(false)
    }, 300)
  }
  // 自动刷新和人为点击刷新
  useEffect(() => {
    const buttonElement = document.getElementById('JobDetailListRefreshBtn')
    if (buttonElement) {
  ...
  async function deleteNode(id: string) {
    ...
    await server.delete([node.path], false, isCloud)
      ...
      <div className='list_body'>
        <Table
          ...
          props={{
            ...
            loading: artificialClickLoading
              ? true
              : noNeedLoading
              ? false
              : loading,
          ...
          columns={[
            {
              ...
              cell: {
                ...
                render({ rowData }) {
                  return <Name key={rowData['id']} nodeId={rowData['id']} userName={userName} />
                }
              }
            },
            {
