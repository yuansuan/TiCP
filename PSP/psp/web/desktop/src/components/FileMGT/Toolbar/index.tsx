...
export const Toolbar = observer(function Toolbar(props: any) {
  ...
  const { job } = jobStore
  const { isSyncToLocal, userName } = props
  const { searchKey } = store
  useEffect(() => {
    store.setFilterRegexp(job?.file_filter_regs)
  })
  ...
  return (
    <StyledLayout>
      <div className='left'>
        ...
        {store.getWidget('download') || (
          <Download disabled={emptyDisabled} userName={userName} />
        )}
        {/* {store.getWidget('move') || <Move disabled={emptyDisabled} />}
        {store.getWidget('delete') || <Delete disabled={emptyDisabled} />} */}
        {store.getWidget('refresh') || (
          <Refresh isSyncToLocal={isSyncToLocal} userName={userName} />
        )}
      </div>
      <div className='right'>
        <Input.Search
