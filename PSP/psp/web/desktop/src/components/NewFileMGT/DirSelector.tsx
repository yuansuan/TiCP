...
export const DirSelector = observer(function DirSelector({
  ...
  const state = useLocalStore(() => ({
    ...
    get okDisable() {
      ...
      if (this.selectedKeys[0]) {
        let node = dirTree
          .filterFirstNode(item => item.id === this.selectedKeys[0])
        
        if (disabledCheckedPaths.includes(node?.path)) {
          return `目录${node.name === '.' ? 'HOME' : node.name}禁止使用，请选择其它目录` 
        }
      }
  ...
  function mapNode(node) {
    ...
    const disabled = disabledPaths.includes(node.path)
...
export function SelectorWithStore({
  store,
  onCancel,
  onOk,
  disabledPaths = [],
  ...
  return (
    <Context.Provider value={finalStore}>
      <DirSelector
        ...
        disabledCheckedPaths={disabledCheckedPaths}
      />
    </Context.Provider>
  )
}
...
export const showDirSelector = (options?: {
  ...
  return Modal.show({
    title: options?.title ? options?.title : '选择目录' ,
    footer: null,
