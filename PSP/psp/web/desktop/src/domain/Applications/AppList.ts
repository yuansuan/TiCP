...
interface IAppList {
  list: Map<string, App>
  zoneList: string[]
  queueList: Array<{ cpu_number: number; queue_name: string; select: boolean }>
  licenseList: Array<{ id: string; name: string; select: boolean }>
}

export default class AppList implements IAppList {
  ...
  fetchTemplates = (params?: { state?: string }) =>
    Http.get('/app/template/list', {
      ...
      runInAction(() => {
        this.list = new Map(
          res.data?.apps?.map(item => [item.name, new App(item)])
        )
      })
  ...
  fetchZoneAreaList = async () => {
    const res = await Http.get('/app/zone')
    this.zoneList = res?.data
  }

  fetchQueueList = async () => {
    const res = await Http.get('/app/queue')
    this.queueList = res?.data?.queues || []
  }
  fetchLicenseList = async () => {
    const res = await Http.get('/app/license')
  ...
  delete = name =>
    Http.delete('/app/template', {
      params: { name, compute_type: 'local' }
    }).then(res => {
      this.fetchTemplates()
