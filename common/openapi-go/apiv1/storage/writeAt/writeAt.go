...
func New(hc *xhttp.Client) API {
	return func(options ...Option) (*writeAt.Response, error) {
		...
		if err != nil {
			logging.Default().Errorf("read compress data err, err: %v", err)
			res := v20230530.Response{ErrorCode: common2.InvalidCompressor, ErrorMsg: "invalid compressor read"}
			jsonBytes, _ := json.Marshal(res)
			return &writeAt.Response{Response: res}, errors.Errorf("http: %v, body: %v", "400 Bad Request", string(jsonBytes))
		}
		resolver := hc.Prepare(xhttp.NewPostRequestBuilder().
			URI("/api/storage/writeAt").
		...
		if err == nil {
			ret.Data = &writeAt.Data{TransferSize: int64(len(buff))}
		}
		return ret, err
	}
}
